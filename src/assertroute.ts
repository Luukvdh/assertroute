 ...
 */
export function assertPromiseLike<T = unknown>(
  x: unknown,
  message = "Expected Promise-like",
  info?: Record<string, unknown>
): asserts x is PromiseLike<T> {
...
export function assertPresent<T>(
  x: T | null | undefined,
  message = "Expected value present",
  info?: Record<string, unknown>
): asserts x is T {
...
export function assertInstanceOf<C extends new (...args: any[]) => any>(
  ...
  message?: string,
  info?: Record<string, unknown>
): asserts x is InstanceType<C> {
  assert(
    isInstanceOf(x, ctor),
    message ?? `Expected instance of ${ctor?.name ?? "<ctor>"}`,
    { ...info, got: (x as any)?.constructor?.name ?? typeof x }
  );
}
 ...
 */
export function assertRoute<T>(
  fallback: T,
  fn: () => T,
  options?: AssertRouteOptions
): T;
export function assertRoute<T, A extends any[]>(
  fallback: T,
  fn: (...args: A) => T,
  options?: AssertRouteOptions
): (...args: A) => T;
...
export function routeWith<T>(fallback: T, options?: AssertRouteOptions) {
  return <A extends any[]>(fn: (...args: A) => T) =>
    assertRoute(fallback, fn, options);
}
 ...
 * @param options Control catching/logging behavior
 */
export function assertRouteAsync<T>(
  fallback: T,
  fn: () => Promise<T>,
  options?: AssertRouteOptions
): Promise<T>;
...
export function assertStringLength(
  ...
  assert(
    (x as string).length === len,
    message ?? `Expected string length ${len}`
  );
}

/** Asserts that x is a string with length >= `n`. */
export function assertStringLengthAtLeast(
  x: unknown,
  n: number,
  message?: string
): asserts x is string {
  ...
  assert(
    (x as string).length >= n,
    message ?? `Expected string length >= ${n}`
  );
}

/** Asserts that x is a string with length <= `n`. */
export function assertStringLengthAtMost(
  x: unknown,
  n: number,
  message?: string
): asserts x is string {
...
export function assertStringLengthBetween(
  ...
  assertString(
    x,
    message ?? `Expected string length between ${min} and ${max}`
  );
...
export function assertStringEqualsIgnoreCase(
  ...
  assert(
    (x as string).toLowerCase() === expected.toLowerCase(),
    message ?? `Expected "${expected}" (case-insensitive)`
  );
}
...
export function assertStringTrimmedNotEmpty(
  x: unknown,
  message = "Expected non-empty (trimmed)"
): asserts x is string {
  assertString(x, message);
...
export function assertNumberGreaterThan(
  x: unknown,
  n: number,
  message?: string
): asserts x is number {
...
export function assertNumberGreaterOrEqual(
  x: unknown,
  n: number,
  message?: string
): asserts x is number {
...
export function assertNumberLessThan(
  x: unknown,
  n: number,
  message?: string
): asserts x is number {
...
/** Asserts that x is a number <= n. */
export function assertNumberLessOrEqual(
  x: unknown,
  n: number,
  message?: string
): asserts x is number {
...
export function assertArrayLength<T = unknown>(
  x: unknown,
  len: number,
  message?: string
): asserts x is T[] {
...
export function assertArrayHasAnyOf<T = unknown>(
  x: unknown,
  items: string[],
  message?: string
): asserts x is T[] {
  ...
  assert(
    ok,
    message ?? `Expected array to contain any of [${items.join(", ")}]`
  );
}

/** Asserts that x is an array containing all the provided items (by string form). */
export function assertArrayHasEveryOf<T = unknown>(
  x: unknown,
  items: string[],
  message?: string
): asserts x is T[] {
...
export function assertArrayItemIsBoolean<T = unknown>(
  x: unknown,
  i: number,
  message?: string
): asserts x is T[] {
  assertArray<T>(x, message ?? `Expected array`);
  assert(
    typeof (x as any[])[i] === "boolean",
    message ?? `Expected boolean at ${i}`
  );
}

/** Asserts that x is an array and element at index i is a string. */
export function assertArrayItemIsString<T = unknown>(
  x: unknown,
  i: number,
  message?: string
): asserts x is T[] {
...
export function assertArrayItemIsNumber<T = unknown>(
  ...
  assertArray<T>(x, message ?? `Expected array`);
  assert(
    typeof (x as any[])[i] === "number",
    message ?? `Expected number at ${i}`
  );
}

/** Asserts that x is an array and element at index i is a plain object. */
export function assertArrayItemIsObject<T = unknown>(
  x: unknown,
  i: number,
  message?: string
): asserts x is T[] {
...
export function assertArrayIncludesNumber<T = unknown>(
  x: unknown,
  needle: number,
  message?: string
): asserts x is T[] {
...
export function assertArrayIncludesObject<T = unknown>(
  x: unknown,
...
export function assertArrayOnlyHasObjects<T = unknown>(
  x: unknown,
  message?: string
): asserts x is Record<string, unknown>[] {
  assertArray<T>(x, message ?? `Expected array`);
...
export function assertArrayOnlyHasStrings<T = unknown>(
  x: unknown,
  message?: string
): asserts x is string[] {
  assertArray<T>(x, message ?? `Expected array`);
...
export function assertArrayOnlyHasNumbers<T = unknown>(
  x: unknown,
  message?: string
): asserts x is number[] {
  assertArray<T>(x, message ?? `Expected array`);
...
export function assertArrayEveryIsFalsy<T = unknown>(
  x: unknown,
  message?: string
): asserts x is T[] {
  assertArray<T>(x, message ?? `Expected array`);
...
export function assertArrayEveryIsTruthy<T = unknown>(
  x: unknown,
  message?: string
): asserts x is T[] {
  assertArray<T>(x, message ?? `Expected array`);
...
export function assertArrayIncludesCondition<T = unknown>(
  x: unknown,
  ...
  assertArray<T>(x, message ?? `Expected array`);
  assert(
    (x as any[]).some(predicate),
    message ?? `Expected array to include an item matching condition`
  );
}
...
>(
  ...
  key: K,
  message?: string
): asserts obj is O & Record<K, unknown> {
...
>(obj: unknown, ...keys: K): asserts obj is O & { [P in K[number]]: unknown } {
...
export function assertKeyEquals<
  O extends Record<string, unknown>,
  K extends keyof O,
>(obj: unknown, key: K, expected: unknown, message?: string): asserts obj is O {
  assertObject(obj, message ?? `Expected object`);
  assert(
    (obj as any)[key] === expected,
    message ??
      `Expected key "${String(key)}" to equal ${JSON.stringify(expected)}`
  );
}
...
export function assertSameKeys(
  ...
  assert(
    ...
    message ?? `Expected same keys`
  );
}

/** Asserts that every value in obj is falsy. */
export function assertAllKeysFalsy(
  obj: unknown,
  message?: string
): asserts obj is Record<string, unknown> {
  assertObject(obj, message ?? `Expected object`);
...
export function assertAllKeysSet(
  obj: unknown,
  message?: string
): asserts obj is Record<string, unknown> {
  assertObject(obj, message ?? `Expected object`);
...
export function assertAnyKeyNull(
  obj: unknown,
  message?: string
): asserts obj is Record<string, unknown> {
  assertObject(obj, message ?? `Expected object`);
...
): asserts x is Element {
  assert(isElement(x), message);
}

/** Asserts that the element has at least one child node/element. */
export function assertElementHasChildren(
  x: unknown,
  message = "Expected element to have children"
): asserts x is Element {
...
export function assertElementHasChild(
  x: unknown,
  message = "Expected element to have a child"
): asserts x is Element {
  assertElement(x, message);
  const el = x as Element;
...
export function assertElementHasChildMatching(
  x: unknown,
  selector: string,
  message?: string
): asserts x is Element {
...
export function assertElementHasDescendant(
  x: unknown,
  selector: string,
  message?: string
): asserts x is Element {
...
export function assertElementAttributeEquals(
  x: unknown,
  name: string,
  expected: string,
  message?: string
  ...
  assert(
    val === expected,
    message ?? `Expected attribute "${name}" to equal "${expected}"`
  );
}
...
export function isElementHidden(x: unknown): x is Element {
  ...
  const computed =
    typeof window !== "undefined" ? window.getComputedStyle(x) : null;
  return computed
...
export function assertDateEarlier(
  x: unknown,
  than: Date,
  message?: string
): asserts x is Date {
  ...
  assert(
    (x as Date).getTime() < than.getTime(),
    message ?? `Expected date earlier than ${than.toISOString?.() ?? than}`
  );
}

/** Asserts that x is a Date later than `than`. */
export function assertDateLater(
  x: unknown,
  than: Date,
  message?: string
): asserts x is Date {
  ...
  assert(
    (x as Date).getTime() > than.getTime(),
    message ?? `Expected date later than ${than.toISOString?.() ?? than}`
  );
}

/** Asserts that x is a Date within [min, max]. */
export function assertDateBetween(
  x: unknown,
  min: Date,
  max: Date,
  message?: string
  ...
  assert(
    t >= min.getTime() && t <= max.getTime(),
    message ??
      `Expected date between ${min.toISOString?.() ?? min} and ${max.toISOString?.() ?? max}`
  );
}

/** Asserts that x is a Date whose full year equals `year`. */
export function assertDateYear(
  x: unknown,
  year: number,
  message?: string
): asserts x is Date {
  assertDate(x, message ?? `Expected Date`);
  assert(
    (x as Date).getFullYear() === year,
    message ?? `Expected year ${year}`
  );
}
...
export function assertNumberNotZero(
  x: unknown,
  message = "Expected non-zero number"
): asserts x is number {
...
export function assertNumberPositive(
  x: unknown,
...
export function assertNumberNonNegative(
  x: unknown,
...
export function assertNumberNegative(
  x: unknown,
...
/** Asserts that x is a number <= 0. */
export function assertNumberNonPositive(
  x: unknown,
...
export function assertNumberInteger(
  x: unknown,
...
export function assertNumberSafeInteger(
  x: unknown,
...
export function assertNumberApproxEquals(
  x: unknown,
  expected: number,
  epsilon = 1e-9,
  message?: string
  ...
  assert(
    Math.abs((x as number) - expected) <= epsilon,
    message ?? `Expected approximately ${expected} ± ${epsilon}`
  );
}
...
export function assertObjectArrayAllHaveKey<T = Record<string, unknown>>(
  x: unknown,
  key: string,
  message?: string
): asserts x is T[] {
...
export function assertObjectArrayEveryHasKeys<T = Record<string, unknown>>(
  x: unknown,
  ...keys: string[]
): asserts x is T[] {
...
export function assertOneOfPrimitive<T extends string | number | boolean>(
  x: unknown,
  options: readonly T[],
  message?: string
): asserts x is T {
  assert(
    options.includes(x as T),
 ...
 */
export function assertOnFail<T>(
  fn: () => T,
  onFail: (err: AssertError) => T,
  options: AssertOnFailOptions = {}
): T {
...
export function onFail<T>(fn: () => T) {
  return {
    ...
    run: (handler: (err: AssertError) => T, options?: AssertOnFailOptions) =>
      assertOnFail(fn, handler, options),
  } as const;
}
...
export function assertSubset(
  ...
  for (const [k, v] of Object.entries(subset)) {
    ...
    assert(
      deepEqual((r as any)[k], v),
      message ?? `Mismatched value at key: ${k}`
    );
  }
}
...
export function assertMapHasKey<K, V>(
  m: unknown,
  key: K,
  message?: string
): asserts m is Map<K, V> {
...
export function assertSetHasValue<T>(
  s: unknown,
  value: T,
  message?: string
): asserts s is Set<T> {
    ...
    if (typeof rule === "function") {
      ...
      switch (rule) {
        case "string":
          assert(
            typeof v === "string",
            message ?? `Expected ${k} to be string`
          );
          break;
        case "number":
          assert(
            typeof v === "number" && Number.isFinite(v as number),
            message ?? `Expected ${k} to be number`
          );
          break;
        case "boolean":
          assert(
            typeof v === "boolean",
            message ?? `Expected ${k} to be boolean`
          );
          break;
        case "object":
          assert(
            typeof v === "object" && v !== null && !Array.isArray(v),
            message ?? `Expected ${k} to be object`
          );
          break;
        case "array":
...
export function assertStringEqualsCanonical(
  x: unknown,
  expected: string,
  message?: string
): asserts x is string {
...
export function assertStringContainsCanonical(
  x: unknown,
  needle: string,
  message?: string
): asserts x is string {
...
export function confirmWithError(
  ...assertions: (() => void)[]
): { ok: true } | { ok: false; error: AssertError } {
  try {
...
export function confirmAll(...assertions: (() => void)[]): {
  ok: boolean;
  errors: AssertError[];
} {
...
export class NarrowingBuilder<V> {
  ...
  check<V2 extends V>(guard: Guard<V, V2>): NarrowingBuilder<V2> {
    ...
    const next = new NarrowingBuilder<V2>(this.value as unknown as V2, [
      ...this.guards,
      guard,
    ]);
    return next;
 ...
 */
export function onConfirmedWith<V>(
  value: V,
  ...guards: Array<Guard<any, any>>
): NarrowingBuilder<V>;
export function onConfirmedWith<V, V1 extends V>(
  value: V,
  g1: Guard<V, V1>
): NarrowingBuilder<V1>;
export function onConfirmedWith<V, V1 extends V, V2 extends V1>(
  value: V,
  g1: Guard<V, V1>,
  g2: Guard<V1, V2>
): NarrowingBuilder<V2>;
export function onConfirmedWith<V, V1 extends V, V2 extends V1, V3 extends V2>(
  value: V,
  g1: Guard<V, V1>,
  g2: Guard<V1, V2>,
  g3: Guard<V2, V3>
): NarrowingBuilder<V3>;
export function onConfirmedWith<
  V,
  V1 extends V,
  V2 extends V1,
  V3 extends V2,
...
(assert as any).instanceOf = <C extends new (...args: any[]) => any>(
  x: unknown,
  ctor: C,
  message?: string
) => assertInstanceOf(x, ctor, message);
(assert as any).isNonEmptyString = (x: unknown, message?: string) =>
  assertNonEmptyString(x, message);
(assert as any).stringLengthAtLeast = (
  x: unknown,
  n: number,
  message?: string
) => assertStringLengthAtLeast(x, n, message);
(assert as any).stringLengthAtMost = (
  x: unknown,
  n: number,
  message?: string
) => assertStringLengthAtMost(x, n, message);
(assert as any).stringContains = (
  x: unknown,
  needle: string | RegExp,
  message?: string
) => assertStringContains(x, needle, message);
(assert as any).isNonEmptyArray = (x: unknown, message?: string) =>
  assertArrayNotEmpty(x, message);
(assert as any).arrayLength = (x: unknown, len: number, message?: string) =>
  assertArrayLength(x, len, message);
